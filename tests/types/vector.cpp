...
TEST(VectorSerialization, VectorPrimitiveCache) {
	AmfVector<int> vi({1, 2, 3}, false);
	AmfVector<int> vif({1, 2, 3}, true);
	AmfVector<unsigned int> vu({1, 2, 3}, false);
	AmfVector<unsigned int> vuf({1, 2, 3}, true);
	AmfVector<double> vd({1.0, 2.0, 3.0}, false);
	AmfVector<double> vdf({1.0, 2.0, 3.0}, true);
	...
	isEqual({ 0x0d, 0x00 }, vi.serialize(ctx));
	isEqual({ 0x0d, 0x02 }, vif.serialize(ctx));
	isEqual({ 0x0e, 0x04 }, vu.serialize(ctx));
	isEqual({ 0x0e, 0x06 }, vuf.serialize(ctx));
	isEqual({ 0x0f, 0x08 }, vd.serialize(ctx));
	isEqual({ 0x0f, 0x0a }, vdf.serialize(ctx));
}

TEST(VectorSerialization, VectorObjectCache) {
	...
	isEqual({ 0x10, 0x00 }, v.serialize(ctx));
}

TEST(VectorSerialization, VectorItemObjectCache) {
	AmfVector<AmfInteger> v({1}, "", false);
	AmfVector<AmfBool> v2({}, "f", true);

	SerializationContext ctx;
	isEqual({ 0x10, 0x03, 0x00, 0x01, 0x04, 0x01 }, v.serialize(ctx));
	isEqual({ 0x10, 0x01, 0x01, 0x03, 0x66 }, v2.serialize(ctx));
	isEqual({ 0x10, 0x00 }, v.serialize(ctx));;
	isEqual({ 0x10, 0x02 }, v2.serialize(ctx));;
}

TEST(VectorSerialization, VectorIntVector) {
	AmfVector<AmfVector<int>> v({ AmfVector<int> { {1, 2, 3 }, false } }, "", false);
	AmfVector<AmfVector<int>> v2({ AmfVector<int> { {1, 2, 3 }, false } }, "", true);
	...
	isEqual({ 0x10, 0x00 }, v.serialize(ctx));
	isEqual({ 0x10, 0x04 }, v2.serialize(ctx));
}

TEST(VectorSerialization, SelfReference) {
	AmfItemPtr ptr(AmfVector<AmfItem>("", false));
	ptr.as<AmfVector<AmfItem>>().values.push_back(ptr);
...
TEST(VectorSerialization, SelfReferenceConcreteType) {
	AmfItemPtr ptr(AmfVector<AmfVector<AmfItem>>({}, "", false));
	ptr.as<AmfVector<AmfVector<AmfItem>>>().values.push_back(ptr);
